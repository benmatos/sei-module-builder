...
>>>>>>> 9df567fe77b95b17e7f8f44c48245904ab97ccfb
...
async def w1_post(request: Request, nome: str = Form(...), slug: str = Form(...),
                   ...
                   versao: str = Form(...), sei_versao_min: str = Form(...), autor: str = Form(...)):
...
async def projeto_exportar(projeto_id: int):
    ...
    content = json.dumps(projeto["definicao"], ensure_ascii=False, indent=2)
...
async def importar_get(request: Request):
    return templates.TemplateResponse(request, "projetos/importar.html", context={
        "flashes": get_flashes(request),
    })
...
def _carregar_definicao_na_sessao(request: Request, definicao: ModuloDefinicao):
    ...
    request.session["step1"] = {
        k: d[k] for k in ("nome", "slug", "namespace", "descricao",
                           "versao", "sei_versao_min", "autor")
    }
    request.session["step2"] = [t for t in d.get("tabelas", [])]
    request.session["step3_recursos"] = d.get("recursos", [])
    request.session["step3_menus"] = d.get("menus", [])
